...
test('e2e: addition', () => assert.strictEqual(run('let x = 1 + 2; x;'), 3));
test('e2e: subtraction', () => assert.strictEqual(run('let x = 10 - 4; x;'), 6));
test('e2e: multiplication', () => assert.strictEqual(run('let x = 3 * 4; x;'), 12));
test('e2e: division', () => assert.strictEqual(run('let x = 8 / 2; x;'), 4));
test('e2e: operator precedence', () =>
  assert.strictEqual(run('let x = 2 + 3 * 4; x;'), 14));
test('e2e: parentheses override precedence', () =>
  assert.strictEqual(run('let x = (2 + 3) * 4; x;'), 20));
test('e2e: unary negation', () => assert.strictEqual(run('let x = -5; x;'), -5));
test('e2e: chained arithmetic', () =>
  assert.strictEqual(run('let a = 2; let b = a * 3; let c = b + 1; c;'), 7));
test('e2e: constant folding addition', () =>
  assert.ok(jsOf('let x = 1 + 2;').includes('= 3')));
test('e2e: constant folding multiplication', () =>
  assert.ok(jsOf('let x = 3 * 4;').includes('= 12')));
test('e2e: constant folding subtraction', () =>
  assert.ok(jsOf('let x = 10 - 3;').includes('= 7')));
test('e2e: constant folding division', () =>
  assert.ok(jsOf('let x = 6 / 2;').includes('= 3')));
test('e2e: constant folding nested', () =>
  assert.ok(jsOf('let x = (1 + 2) * (3 + 4);').includes('= 21')));
...
test('e2e: logical and', () =>
  assert.strictEqual(run('let x = true && false; x;'), false));
test('e2e: logical or', () =>
  assert.strictEqual(run('let x = false || true; x;'), true));
test('e2e: logical not', () => assert.strictEqual(run('let x = !true; x;'), false));
test('e2e: not false is true', () => assert.strictEqual(run('!false;'), true));
test('e2e: constant folding && true true', () =>
  assert.ok(jsOf('let x = true && true;').includes('= true')));
test('e2e: constant folding || false false', () =>
  assert.ok(jsOf('let x = false || false;').includes('= false')));
test('e2e: constant folding ! true', () =>
  assert.ok(jsOf('let x = !true;').includes('= false')));
...
test('e2e: constant folding comparison', () =>
  assert.ok(jsOf('let x = 1 < 2;').includes('= true')));

// ── strings ───────────────────────────────────────────────────────────────────

test('e2e: string literal', () =>
  assert.strictEqual(run('let s = "hello"; s;'), 'hello'));
test('e2e: string concatenation', () =>
  assert.strictEqual(run('let s = "hello" + " world"; s;'), 'hello world'));
test('e2e: empty string', () => assert.strictEqual(run('let s = ""; s;'), ''));

// ── variables ─────────────────────────────────────────────────────────────────

test('e2e: let and read', () => assert.strictEqual(run('let x = 42; x;'), 42));
test('e2e: assignment updates value', () =>
  assert.strictEqual(run('let x = 1; x = 99; x;'), 99));
test('e2e: multiple variables', () =>
  assert.strictEqual(run('let a = 1; let b = 2; a + b;'), 3));
...
test('e2e: recursive-style function', () =>
  assert.strictEqual(
    run('mine double(n) { return n * 2; } let r = double(double(3)); r;'),
    12
  ));
...
test('e2e: function uses outer let', () =>
  assert.strictEqual(run('mine id(x) { return x; } let r = id(77); r;'), 77));
...
test('e2e: if true branch taken', () =>
  assert.strictEqual(run('let x = 0; if (true) { x = 1; } x;'), 1));
test('e2e: if false branch not taken', () =>
  assert.strictEqual(run('let x = 0; if (false) { x = 1; } x;'), 0));
test('e2e: if-else true', () =>
  assert.strictEqual(run('let x = 0; if (true) { x = 1; } else { x = 2; } x;'), 1));
test('e2e: if-else false', () =>
  assert.strictEqual(run('let x = 0; if (false) { x = 1; } else { x = 2; } x;'), 2));
test('e2e: if with comparison', () =>
  assert.strictEqual(run('let x = 5; let r = 0; if (x > 3) { r = 1; } r;'), 1));
test('e2e: nested if', () =>
  assert.strictEqual(
    run('let x = 10; let r = 0; if (x > 5) { if (x > 8) { r = 2; } } r;'),
    2
  ));
...
test('e2e: while loop runs', () =>
  assert.strictEqual(run('let i = 0; while (i < 3) { i = i + 1; } i;'), 3));
test('e2e: while loop does not run if false', () =>
  assert.strictEqual(run('let i = 5; while (false) { i = 0; } i;'), 5));
...
test('e2e: rejects syntax error', () =>
  assert.throws(() => compile('let = 1;'), CubescriptError));

// ── pipeline stages ───────────────────────────────────────────────────────────

test('e2e: parse produces Program node', () =>
  assert.strictEqual(parse('let x = 1;').kind, 'Program'));
test('e2e: analyze returns analyzed flag', () =>
  assert.strictEqual(analyze(parse('let x = 1;')).analyzed, true));
test('e2e: optimize returns optimized flag', () =>
  assert.strictEqual(optimize(analyze(parse('let x = 1;'))).optimized, true));
test('e2e: generate returns string', () =>
  assert.strictEqual(
    typeof generate(optimize(analyze(parse('let x = 1;')))),
    'string'
  ));
test('e2e: compile without optimize skips folding', () => {
  const { code } = compile('let x = 1 + 2;', { optimize: false });
